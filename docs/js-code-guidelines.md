## JavaScript
If possible don't use jQuery. Anyways if your project still requires some jQuery be sure to use component based modular approach!
...
* Always use `class`. Avoid manipulating `prototype` directly. `class` syntax is more concise and easier to reason about.
...
* Do not use wildcard imports.
    ...
    ```
* Only import from a path in one place. Having multiple lines that import from the same path can make code harder to maintain.
